...
static void __attribute__((noreturn)) cow_entry(struct Trapframe* tf) {
    ...
    if (!(perm & PTE_COW)) {
        user_panic("virtual address %d doesn't have 'PTE_COW'", &va);
    }
...
static void duppage(u_int envid, u_int vpn) {
    ...
    addr = vpn << PGSHIFT;
    /* Step 2: If the page is writable, and not shared with children, and
     * not marked as COW yet, then map it as copy-on-write, both in the
     * parent (0) and the child (envid). */
    /* Hint: The page should be first mapped to the child before remapped in
     * the parent. (Why?)
     */
...
int fork(void) {
    ...
    for (i = 0; i < VPN(USTACKTOP); i += 1) {
        if ((vpd[i >> 10] & PTE_V) && (vpt[i] & PTE_V)) {
            duppage(child, i);
        }
    }
    /* Step 4: Set up the child's tlb mod handler and set child's
     * 'env_status' to 'ENV_RUNNABLE'. */
    /* Hint:
     *   You may use 'syscall_set_tlb_mod_entry' and
     * 'syscall_set_env_status' Child's TLB Mod user exception entry should
     * handle COW, so set it to 'cow_entry'
     */
